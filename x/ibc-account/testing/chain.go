...
import (
	...
	"github.com/chainapsis/cosmos-sdk-interchain-account/x/ibc-account/testing/mock"
...
const (
	...
	IBCAccountPort = ibcaccounttypes.ModuleName
	MockPort       = mocktypes.ModuleName
...
// NOTE: only creation of a capbility for a ibc-account or mock port is supported
...
func (chain *TestChain) CreatePortCapability(portID string) {
	...
	if !ok {
		...
		switch portID {
		case IBCAccountPort:
			// claim capability using the ibc-account capability keeper
			err = chain.App.ScopedIBCAccountKeeper.ClaimCapability(chain.GetContext(), cap, host.PortPath(portID))
			require.NoError(chain.t, err)
		default:
...
func (chain *TestChain) CreateChannelCapability(portID, channelID string) {
	...
	if !ok {
		...
		require.NoError(chain.t, err)
		err = chain.App.ScopedIBCAccountKeeper.ClaimCapability(chain.GetContext(), cap, capName)
		require.NoError(chain.t, err)
	}
...
//
// NOTE: does not work with ibc-account module
